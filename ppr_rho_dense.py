...
STOPPING CRITERION:
- We terminate using the proximal-gradient fixed-point residual (unit step):
      T_{alpha,rho}(x) = prox_g(x - ∇f(x))
      r(x) = ||x - T_{alpha,rho}(x)||_∞
  and measure work to reach r(x_k) <= eps (here: eps_gap variable, unchanged name).
...
# -----------------------------
# NEW: prox-gradient fixed-point residual (termination metric)
# -----------------------------
def prox_map_T(x: np.ndarray, Px: np.ndarray, *, c: float, tau: np.ndarray, b_seed: float, seed: int) -> np.ndarray:
    """
    Fixed-point mapping with unit step:
        T(x) = prox_g( x - ∇f(x) )

    For this objective:
        x - ∇f(x) = c * (x + P x) + b_seed * e_seed
    and prox_g is weighted soft-threshold with tau = rho*alpha*sqrt(d).
    """
    u = c * (x + Px)
    u[seed] += b_seed
...
# -----------------------------
# ISTA / FISTA (same logic, NEW termination metric)
# -----------------------------
def run_ista_history(P, d, seed, alpha, rho, max_iter):
    ...
    # Record prox-fixed-point residual at x0
    ...
    for k in range(1, max_iter + 1):
        ...
        # Record prox-fixed-point residual at current iterate x_k
        Tx = prox_map_T(x, Px, c=c, tau=tau, b_seed=b_seed, seed=seed)
...
def run_fista_until(P, d, seed, alpha, rho, beta, target, max_iter):
    ...
    # Residual at x0
    Tx = prox_map_T(x, Px, c=c, tau=tau, b_seed=b_seed, seed=seed)
    f = fixed_point_residual_inf(x, Tx)
    if f <= target:
        return 0, 0.0, f, True
    ...
    for k in range(1, max_iter + 1):
        ...
        f = fixed_point_residual_inf(x, Tx)
        if f <= target:
            return k, work, f, True

    return max_iter, work, f, False
...
def rho_sweep(
    ...
    *,
    ...
    for i, rho in enumerate(rho_grid):
        ...
        # ISTA residual history (up to ista_ref) and first-hit work for r(x)<=eps_gap
        objs_I, works_I = run_ista_history(P, d, seed_node, alpha, rho, max_iter=ista_ref)
        work_I = first_work_below(objs_I, works_I, eps_gap)

        # FISTA work to reach the same residual tolerance (eps_gap)
        itF, work_F, fF, reached = run_fista_until(P, d, seed_node, alpha, rho, beta, eps_gap, max_iter=fista_cap)
...
def main():
    ...
    # Theoretical thresholds (based on fixed degrees in this construction)
    d_core = 59 + c_boundary_per_core
    lhs_const = nopercolation_LHS(alpha, beta, eta, d_core, d_core)
    rho_min_noperc = lhs_const / alpha
    rho_zero = 1.0 / d_core

    # Pick rho grid so that no-percolation holds for ALL points, and we cross into x*=0 regime.
    rho_lo = 1e-4
    if rho_lo < rho_min_noperc:
        rho_lo = 1.05 * rho_min_noperc

    rho_hi = 5e-2  # safely above rho_zero (~1/79)
    rho_grid = np.logspace(math.log10(rho_lo), math.log10(rho_hi), 20)
    ...
    # Keep x-axis log labels as 10^k (recommended). If you want K-style on x too, say so.
    ...
    # ax.axvline(rho_zero, linestyle=":",  linewidth=4, alpha=0.8, label="rho (x*=0)")

    ax.legend(loc="lower center", fontsize=32)
    fig.tight_layout()

    fig.savefig("B600_work_vs_rho.png", dpi=200)
    fig.savefig("B600_work_vs_rho.pdf")
    print("[saved] B600_work_vs_rho.png and B600_work_vs_rho.pdf")
    plt.show()
